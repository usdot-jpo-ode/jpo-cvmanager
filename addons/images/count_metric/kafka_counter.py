...
class KafkaMessageCounter:
  def __init__(self, thread_id, message_type, rsu_location_dict, rsu_count_dict, rsu_count_dict_zero, type):
  ...
  def process_message(self, message):
    try:
      ...
      if self.type == 0:
        ...
        for msg_content in item_arr:
          if 'originRsu' in msg_content['metadata']:
            ...
            logging.warning(f'{self.thread_id}: Malformed message detected. No source IP.')
            originIp = "noIP"
      ...
      logging.error(f'{self.thread_id}: A Kafka message failed to be processed with the following error: {e}')
  ...
  def start_counter(self):
    ...
    scheduler.add_job(self.push_metrics, 'cron', minute=f"*/{str(self.count_window)}")
    scheduler.start()
