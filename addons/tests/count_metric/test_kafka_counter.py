...
from images.count_metric import kafka_counter
...
def createKafkaMessageCounter(type: int):
    ...
    kafka_counter.bigquery.Client.return_value.query.return_value.result.return_value.total_rows = 1
    thread_id = 0
    message_type = 'bsm'
    rsu_location_dict = {'noIP': 'Unknown'}
    rsu_count_dict = {'Unknown': {'noIP': 1}}
    rsu_count_dict_zero = {'Unknown': {'noIP': 0}}
    newKafkaMessageCounter = kafka_counter.KafkaMessageCounter(thread_id, message_type, rsu_location_dict, rsu_count_dict, rsu_count_dict_zero, type)

    return newKafkaMessageCounter

def test_write_bigquery_with_type0_kmc_success():
    # prepare
    ...
    kafkaMessageCounterType0.client.query.assert_called_once_with(expectedArgument)
    kafkaMessageCounterType0.client.query.return_value.result.assert_called_once()
    kafka_counter.logging.info.assert_called_once()

def test_write_bigquery_with_type1_kmc_success():
    # prepare
    ...
    expectedArgument = f'INSERT INTO `{targetTable}`(RSU, Road, Date, Type, Count) VALUES {query_values}'
    kafkaMessageCounterType1.client.query.assert_called_once_with(expectedArgument)
    kafkaMessageCounterType1.client.query.return_value.result.assert_called_once()
    kafka_counter.logging.info.assert_called_once()
...
def test_push_metrics_exception():
    # prepare
    kafkaMessageCounter = createKafkaMessageCounter(0)
    kafkaMessageCounter.write_bigquery = MagicMock()
...
def test_listen_for_message_and_process_success():
    ...
    kafka_counter.KafkaConsumer.return_value = {
        'test': 'test'
    }
...
def test_read_topic_success():
    ...
    kafka_counter.os.getenv = MagicMock()
    kafka_counter.os.getenv.return_value = 'test'
    ...
    kafkaMessageCounter.listen_for_message_and_process.assert_called_once_with('test', 'test')
