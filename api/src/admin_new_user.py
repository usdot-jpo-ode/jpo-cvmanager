...
def check_safe_input(user_spec):
    ...
    if any(c in special_characters for c in user_spec["first_name"]):
        logging.info(f"check_safe_input: first_name")
        return False
    if any(c in special_characters for c in user_spec["last_name"]):
        logging.info(f"check_safe_input: last_name")
        return False
    for org in user_spec["organizations"]:
        logging.info(f"check_safe_input: organizations")
        if any(c in special_characters for c in org["name"]):
            logging.info(f"check_safe_input: organizations->name")
            return False
        if any(c in special_characters for c in org["role"]):
            logging.info(f"check_safe_input: organizations->role")
            return False
    return True


def add_user(user_spec):
    logging.info(f"Adding user {str(user_spec)}")
    # Check for special characters for potential SQL injection
    if not check_email(user_spec["email"]):
        logging.info(f"BAD EMAIL: %s", user_spec["email"])
        return {"message": "Email is not valid"}, 500
    if not check_safe_input(user_spec):
        logging.info(f"NOT SAFE INPUT: %s", user_spec["email"])
        return {
    ...
    try:
        ...
        logging.info(f"user_insert_query: {user_insert_query}")
        ...
        logging.info(f"user_org_insert_query: {user_insert_query}")
...
class AdminNewUser(Resource):
    ...
    def post(self):
        ...
        if errors:
            logging.info("AdminNewUser failed to validate request: {request.json}")
            logging.error(str(errors))
            abort(400, str(errors))

        logging.info(f"AdminNewUser request: {request.json}")
        data, code = add_user(request.json)
        logging.info(f"data: {data}, code: {code}")
