...
def get_ping_data(organization):
  ...
  query = "SELECT jsonb_build_object('id', rd.rsu_id, 'ip', rd.ipv4_address, 'datetime', ping_data.timestamp, 'online_status', ping_data.result) " \
...
def get_last_online_data(ip, organization):
  ...
  return {
    'ip': ip,
    'last_online': util.format_date_denver(result[0].strftime('%m/%d/%Y %I:%M:%S %p')) if len(result) != 0 else 'No Data'
  }
...
class RsuOnlineStatus(Resource):
  ...
  def get(self):
    ...
    if 'rsu_ip' in request.args:
      return (get_last_online_data(request.args['rsu_ip'], request.environ['organization']), 200, self.headers)
    else:
      return (get_rsu_online_statuses(request.environ['organization']), 200, self.headers)