...
def get_organization_rsus(organization):
  ...
  query = "SELECT jsonb_build_object('ip', rd.ipv4_address) " \
...
class RsuQueryCounts(Resource):
  ...
  def get(self):
    ...
    start = request.args.get('start', default=((datetime.now() - timedelta(1)).strftime("%Y-%m-%dT%H:%M:%S")))
    ...
    if message.upper() not in msgList:
      return ("Invalid Message Type.\nValid message types: SSM, BSM, SPAT, SRM, MAP", 400, self.headers)
    
    rsus = get_organization_rsus(request.environ['organization'])
    data, code = query_rsu_counts(rsus, message.upper(), start, end)
