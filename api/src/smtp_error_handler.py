...
def configure_error_emails(app):
    mail_handler = SMTP_SSLHandler(mailhost=[os.environ['CSM_TARGET_SMTP_SERVER_ADDRESS'], int(os.environ['CSM_TARGET_SMTP_SERVER_PORT'])],
                                   fromaddr=os.environ['CSM_EMAIL_TO_SEND_FROM'],
                                   toaddrs=[],
                                   subject='Automated CV Manager API Error',
                                   credentials=[
                                       os.environ['CSM_EMAIL_APP_USERNAME'], os.environ['CSM_EMAIL_APP_PASSWORD']],
                                   secure=())
    ...
    def emit(self, record):
        try:
            ...
            for email in subscribed_users:
                ...
                EMAIL_KEYS['UNSUBSCRIBE_LINK'] = os.environ["ERROR_EMAIL_UNSUBSCRIBE_LINK"].format(
                    email=email)

                for key, value in EMAIL_KEYS.items():
                    body_content = body_content.replace(f"##_{key}_##", value)
                message.attach(MIMEText(body_content, "html"))
                smtp.sendmail(self.fromaddr, email,
                            message.as_string())
            smtp.quit()

            logging.debug(
                f"Successfully sent error email to {subscribed_users}")
