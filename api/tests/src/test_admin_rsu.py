...
import pytest
import src.admin_rsu as admin_rsu
import tests.data.admin_rsu_data as admin_rsu_data
import sqlalchemy
...
@patch('src.admin_rsu.get_modify_rsu_data')
def test_entry_get_rsu(mock_get_modify_rsu_data):
    ...
    with patch("src.admin_rsu.request", req):
        status = admin_rsu.AdminRsu()
        (body, code, headers) = status.get()

        mock_get_modify_rsu_data.assert_called_once_with(admin_rsu_data.request_args_rsu_good['rsu_ip'])
        assert code == 200
...
def test_entry_get_all(mock_get_modify_rsu_data):
    ...
    with patch("src.admin_rsu.request", req):
        ...
        mock_get_modify_rsu_data.assert_called_once_with(admin_rsu_data.request_args_all_good['rsu_ip'])
        assert code == 200
        assert headers['Access-Control-Allow-Origin'] == "*"
        assert body == {}
...
@patch('src.admin_rsu.modify_rsu')
...
@patch('src.admin_rsu.delete_rsu')
def test_entry_delete_rsu(mock_delete_rsu):
    ...
    with patch("src.admin_rsu.request", req):
        ...
        mock_delete_rsu.assert_called_once_with(admin_rsu_data.request_args_rsu_good['rsu_ip'])
        assert code == 200
        assert headers['Access-Control-Allow-Origin'] == "*"
        assert body == {}
...
@patch('src.admin_rsu.get_rsu_data')
...
@patch('src.admin_rsu.admin_new_rsu.get_allowed_selections')
@patch('src.admin_rsu.get_rsu_data')
...
@patch('src.admin_rsu.admin_new_rsu.check_safe_input')
@patch('src.admin_rsu.pgquery.insert_db')
