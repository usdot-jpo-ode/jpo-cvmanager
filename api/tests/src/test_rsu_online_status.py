...
import src.rsu_online_status as rsu_online_status
import src.util as util
import tests.data.rsu_online_status_data as data
import pytest
...
@patch('src.rsu_online_status.get_last_online_data')
...
@patch('src.rsu_online_status.pgquery')
def test_ping_data_query(mock_pgquery):
    t = datetime.utcnow() - timedelta(minutes=20)
    organization = 'Test'
    expected_query = "SELECT jsonb_build_object('id', rd.rsu_id, 'ip', rd.ipv4_address, 'datetime', ping_data.timestamp, 'online_status', ping_data.result) " \
...
@patch('src.rsu_online_status.get_ping_data')
@patch('src.rsu_online_status.get_last_online_data')
