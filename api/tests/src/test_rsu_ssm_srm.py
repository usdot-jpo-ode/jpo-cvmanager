...
import src.rsu_ssm_srm as rsu_ssm_srm
import tests.data.rsu_ssm_srm_data as ssm_srm_data
...
@patch('src.rsu_ssm_srm.query_ssm_data')
@patch('src.rsu_ssm_srm.query_srm_data')
def test_get_request(mock_srm, mock_ssm):
    ...
    with patch("src.rsu_ssm_srm.request", req):
...
@patch('src.rsu_ssm_srm.bigquery')
@patch('src.rsu_ssm_srm.datetime')
def test_query_ssm_data_query(mock_date, mock_bigquery):
    ...
    with patch.dict('src.rsu_ssm_srm.os.environ', {'SSM_DB_NAME': 'Fake_table'}):
...
def test_query_srm_data_query(mock_date, mock_bigquery):
    mock_bigquery.Client.return_value.query.return_value = []
    mock_date.now.return_value = datetime.strptime('2022/12/14 00:00:00', '%Y/%m/%d %H:%M:%S')
    with patch.dict('src.rsu_ssm_srm.os.environ', {'SRM_DB_NAME': 'Fake_table'}):
