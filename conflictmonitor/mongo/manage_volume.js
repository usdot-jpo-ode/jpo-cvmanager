...
const DB_DELETE_SIZE_BYTES = CM_DATABASE_SIZE_GB * CM_DATABASE_DELETE_THRESHOLD_PERCENT * BYTE_TO_GB
...
function updateTTL() {
  ...
  const newestRecords = db
    .getCollection(CM_DATABASE_STORAGE_COLLECTION_NAME)
    .find()
    .sort({ recordGeneratedAt: -1 })
    .limit(10)
  ...
  newestRecords.forEach((doc) => {
    ...
    for (let i = 0; i < doc.collectionStats.length; i++) {
      total +=
        doc.collectionStats[i].allocatedSpace + doc.collectionStats[i].freeSpace + doc.collectionStats[i].indexSize
    }
