...
def test_setZabbixAuth():
    ...
    # check
    rsu_ping_fetch.logging.info.assert_called_once_with('Fetching Zabbix auth token from endpoint')
    rsu_ping_fetch.requests.post.assert_called_once_with('endpoint', json={
        "jsonrpc": "2.0",
        "method": "user.login",
        "id": 1,
        "params": {
...
def test_getHostInfo():
    ...
    # check
    rsu_ping_fetch.requests.post.assert_called_once_with(rsf.ZABBIX_ENDPOINT, json={
        "jsonrpc": "2.0",
        "method": "host.get",
        "id": 1,
        "auth": 'auth',
...
def test_insertHistoryItem():
    ...
    rsu_ping_fetch.logging.info.assert_called_once_with('Inserting 1 history items for RSU testaddress')
    assert(result == True)
...
def test_run():
    ...
    rsu_ping_fetch.get_rsu_data.return_value = [
        {
            "rsu_id": 1,
            "rsu_ip": "testaddress"
        }
...
def test_run_insert_failure():
    ...
    rsf.getHistory.assert_called_once()
    rsf.insertHistoryItem.assert_called_once()
    rsu_ping_fetch.logging.warning.assert_called_once_with('Failed to insert history item for testaddress')
    rsu_ping_fetch.logging.error.assert_not_called()
...
def test_run_exception():
    ...
    rsf.printConfigInfo.assert_called_once()
    rsu_ping_fetch.get_rsu_data.assert_called_once()
    rsu_ping_fetch.logging.info.assert_called_once_with('Found 1 RSUs to fetch status for')
    rsf.getHostInfo.assert_called_once()
    rsu_ping_fetch.logging.warning.assert_not_called()
    rsu_ping_fetch.logging.error.assert_called_once_with('Failed to fetch Zabbix data RSU testaddress')
