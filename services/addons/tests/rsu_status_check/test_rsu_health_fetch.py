...
from addons.images.rsu_status_check.rsu_health_fetch import main


@patch("os.environ", {"RSU_HEALTH_FETCH": "False"})
@patch("addons.images.rsu_status_check.rsu_health_fetch.UpdatePostgresRsuHealth")
@patch("addons.images.rsu_status_check.rsu_health_fetch.logging")
def test_main_service_disabled(mock_logging, mock_update_pg):
    main()
    mock_logging.info.assert_called_once_with(
        "The rsu-health-fetch service is disabled and will not run"
    )
    mock_update_pg.assert_not_called()


@patch("os.environ", {"RSU_HEALTH_FETCH": "True"})
@patch("addons.images.rsu_status_check.rsu_health_fetch.UpdatePostgresRsuHealth")
@patch("addons.images.rsu_status_check.rsu_health_fetch.logging")
