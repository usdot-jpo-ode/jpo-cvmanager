...
def add_user(user_spec):
    ...
    try:
        user_insert_query = (
            "INSERT INTO public.users(email, first_name, last_name, created_timestamp, super_user) "
            f"VALUES ('{user_spec['email']}', '{user_spec['first_name']}', '{user_spec['last_name']}', {int(time.time() * 1000)}, '{'1' if user_spec['super_user'] else '0'}')"
        )
