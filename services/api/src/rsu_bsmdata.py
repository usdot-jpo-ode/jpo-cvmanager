...
def query_bsm_data(pointList, start, end):
  ...
  query = "SELECT DISTINCT bsm.metadata.originIp as Ip, " \
...
class RsuBsmData(Resource):
  ...
  def post(self):
    ...
    try:
      ...
      return ('Body format: {"start": string, "end": string, "geometry": coordinate list}', 400, self.headers)
    
    data, code = query_bsm_data(pointList, start, end)
