...
def mark_rsu_for_upgrade(rsu_ip):
  ...
  if upgrade_info['upgrade_available'] == False:
    return {"error": f"Requested RSU '{rsu_ip}' is already up to date with the latest firmware"}, 500
  ...
  response = requests.post(f"{firmware_manager_endpoint}/init_firmware_upgrade", json = post_body)
