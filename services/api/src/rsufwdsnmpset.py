...
def config_txrxmsg(rsu_ip, snmp_creds, dest_ip, udp_port, rsu_index, psid, tx):
  try:
    ...
    if tx:
      ...
      snmp_mod += 'NTCIP1218-v01:rsuXmitMsgFwdingPsid.{index} x {msgpsid} '.format(index=rsu_index, msgpsid=psid)
      snmp_mod += 'NTCIP1218-v01:rsuXmitMsgFwdingDestIpAddr.{index} s {destip} '.format(index=rsu_index, destip=dest_ip)
      snmp_mod += 'NTCIP1218-v01:rsuXmitMsgFwdingDestPort.{index} i {port} '.format(index=rsu_index, port=udp_port)
      snmp_mod += 'NTCIP1218-v01:rsuXmitMsgFwdingProtocol.{index} i 2 '.format(index=rsu_index)
      ...
      snmp_mod += 'NTCIP1218-v01:rsuXmitMsgFwdingDeliveryStart.{index} x {dt} '.format(index=rsu_index, dt=start_hex)
      snmp_mod += 'NTCIP1218-v01:rsuXmitMsgFwdingDeliveryStop.{index} x {dt} '.format(index=rsu_index, dt=end_hex)
      snmp_mod += 'NTCIP1218-v01:rsuXmitMsgFwdingSecure.{index} i 0 '.format(index=rsu_index)
      snmp_mod += 'NTCIP1218-v01:rsuXmitMsgFwdingStatus.{index} i 4'.format(index=rsu_index)
    ...
    # rsuReceivedMsgAuthMsgInterval - int : Do not turn on. (0: off, 1: on)
      ...
      snmp_mod = 'snmpset -v 3 {auth} {rsuip} '.format(auth=authstring, rsuip=rsu_ip)
      snmp_mod += 'NTCIP1218-v01:rsuReceivedMsgPsid.{index} x {msgpsid} '.format(index=rsu_index, msgpsid=psid)
      snmp_mod += 'NTCIP1218-v01:rsuReceivedMsgDestIpAddr.{index} s {destip} '.format(index=rsu_index, destip=dest_ip)
      snmp_mod += 'NTCIP1218-v01:rsuReceivedMsgDestPort.{index} i {port} '.format(index=rsu_index, port=udp_port)
      snmp_mod += 'NTCIP1218-v01:rsuReceivedMsgProtocol.{index} i 2 '.format(index=rsu_index)
      snmp_mod += 'NTCIP1218-v01:rsuReceivedMsgRssi.{index} i -100 '.format(index=rsu_index)
      snmp_mod += 'NTCIP1218-v01:rsuReceivedMsgInterval.{index} i 1 '.format(index=rsu_index)
      ...
      snmp_mod += 'NTCIP1218-v01:rsuReceivedMsgDeliveryStart.{index} x {dt} '.format(index=rsu_index, dt=start_hex)
      snmp_mod += 'NTCIP1218-v01:rsuReceivedMsgDeliveryStop.{index} x {dt} '.format(index=rsu_index, dt=end_hex)
      snmp_mod += 'NTCIP1218-v01:rsuReceivedMsgStatus.{index} i 4 '.format(index=rsu_index)
      snmp_mod += 'NTCIP1218-v01:rsuReceivedMsgSecure.{index} i 0 '.format(index=rsu_index)
      snmp_mod += 'NTCIP1218-v01:rsuReceivedMsgAuthMsgInterval.{index} i 0'.format(index=rsu_index)
...
def config_del(rsu_ip, snmp_version, snmp_creds, msg_type, rsu_index):
  if snmp_version == '41':
    try:
      ...
      output = e.stderr.decode("utf-8").split('\n')[:-1]
      logging.error(f'Encountered error while deleting the RSU 4.1 SNMP config: {output[-1]}')
      response = snmperrorcheck.check_error_type(output[-1])
      code = 500
    finally:
      # Put RSU in run mode, this doesn't need to be captured
    ...
    try:
      ...
      if msg_type.lower() == 'bsm':
        snmp_mods += 'NTCIP1218-v01:rsuReceivedMsgStatus.{index} i 6 '.format(index=rsu_index)
      if msg_type.lower() == 'spat':
        snmp_mods += 'NTCIP1218-v01:rsuXmitMsgFwdingStatus.{index} i 6 '.format(index=rsu_index)
      ...
      logging.error(f'Encountered error while deleting NTCIP 1218 SNMP config: {output[-1]}')
      response = snmperrorcheck.check_error_type(output[-1])
      code = 500
  else:
