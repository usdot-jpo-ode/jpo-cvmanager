...
modify_org_sql = "UPDATE public.organizations SET " \
  "name = 'test org' " \
  "WHERE name = 'test org'"

modify_org_add_user_sql = "INSERT INTO public.user_organization(user_id, organization_id, role_id) VALUES" \
  " (" \
...
modify_org_modify_user_sql = "UPDATE public.user_organization " \
...
modify_org_remove_user_sql = "DELETE FROM public.user_organization WHERE " \
  "user_id = (SELECT user_id FROM public.users WHERE email = 'test3@email.com') " \
  "AND organization_id = (SELECT organization_id FROM public.organizations WHERE name = 'test org')"

modify_org_add_rsu_sql = "INSERT INTO public.rsu_organization(rsu_id, organization_id) VALUES" \
  " (" \
...
modify_org_remove_rsu_sql = "DELETE FROM public.rsu_organization WHERE " \
