...
@patch('api.src.rsu_upgrade.check_for_upgrade', return_value={
        "upgrade_available": True,
        "upgrade_id": 2,
        "upgrade_name": "new_fw_name",
        "upgrade_version": "1.0.0"
    })
def test_mark_rsu_for_upgrade_eligible(mock_check_for_upgrade, mock_write_db, mock_requests_post):
    ...
    mock_requests_post.assert_called_with("http://1.1.1.1:8080/init_firmware_upgrade", json={"rsu_ip": "192.168.0.10"})
...
def test_mark_rsu_for_upgrade_eligible_but_rejected(mock_check_for_upgrade, mock_write_db, mock_requests_post):
    mock_response = MagicMock()
    mock_requests_post.return_value = mock_response
    ...
    expected_message, expected_status_code = {"error":"Firmware upgrade request denied due to an upgrade already occurring for the target RSU"}, 500
    assert actual_message == expected_message
    assert actual_status_code == expected_status_code
...
def test_mark_rsu_for_upgrade_ineligible(mock_check_for_upgrade, mock_write_db, mock_requests_post):
    #call function
    rsu_ip = "192.168.0.10"
    actual_message, actual_status_code = rsu_upgrade.mark_rsu_for_upgrade(rsu_ip)
    ...
    expected_message, expected_status_code = {"error": f"Requested RSU '{rsu_ip}' is already up to date with the latest firmware"}, 500
    assert actual_message == expected_message
    assert actual_status_code == expected_status_code