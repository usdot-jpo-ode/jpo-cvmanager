...
class UpdatePostgresRsuMessageForward(UpdatePostgresSnmpAbstractClass):
    ...
    def update_postgresql(self, rsu_snmp_configs_obj, subset=False):
        ...
        if subset:
            recorded_config_list = self.get_config_list(rsu_snmp_configs_obj)
        else:
            recorded_config_list = self.get_config_list()
        msgfwd_types = self.get_msgfwd_types()
        ...
        for snmp_configs in rsu_snmp_configs_obj.values():
            ...
            for snmp_config in snmp_configs:
                if snmp_config not in recorded_config_list:
                    ...
                    # Swap msgfwd_type string with PostgreSQL id
                    snmp_config["msgfwd_type"] = msgfwd_types[
                        snmp_config["msgfwd_type"]
                    ]
                    configs_to_add.append(snmp_config)

        # Make deletions
        if len(configs_to_remove) > 0:
            self.delete_config_list(configs_to_remove)
