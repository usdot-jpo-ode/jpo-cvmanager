...
public class MapMinimumDataEventRepositoryImplTest {
    ...
    @Test
    public void testFindMapMinimumDataEvents() {
        Query query = new Query();
        List<MapMinimumDataEvent> expected = new ArrayList<>();

        Mockito.doReturn(expected).when(mongoTemplate).find(query, MapMinimumDataEvent.class, "CmMapMinimumDataEvents");

        List<MapMinimumDataEvent> results = repository.find(query);
    ...
    public void testGetMapMinimumDataEventsByDay() {
        ...
        List<IDCount> actualResults = repository.getAggregatedDailyMapMinimumDataEventCounts(intersectionID, startTime,
                endTime);
