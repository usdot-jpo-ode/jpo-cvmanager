...
public class SpatBroadcastRateNotificationRepositoryImplTest {
    ...
    @Test
    public void testFindSpatBroadcastRateNotifications() {
        Query query = new Query();
        List<SpatBroadcastRateNotification> expected = new ArrayList<>();

        Mockito.doReturn(expected).when(mongoTemplate).find(query, SpatBroadcastRateNotification.class, "CmSpatBroadcastRateNotification");

        List<SpatBroadcastRateNotification> results = repository.find(query);
