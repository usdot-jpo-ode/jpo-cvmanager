...
it('Test post request', async () => {
  ...
  fetchMock.mockResponseOnce('NOT JSON')
  actualResponse = await ApiHelper._postData({ url: 'https://test.com', token: 'testToken', body: {} })
  expect(actualResponse.body).toEqual(undefined)
})

it('Test post request Error', async () => {
  fetchMock.mockRejectOnce(new Error('fake error message'))
  const actualResponse = await ApiHelper._postData({ url: 'https://test.com', token: 'testToken', body: {} })
  expect(actualResponse).toEqual(null)
})
...
it('Test patch request', async () => {
  const expectedResponse = { data: 'Test JSON' }
  fetchMock.mockResponseOnce(JSON.stringify(expectedResponse))
  let actualResponse = await ApiHelper._patchData({ url: 'https://test.com', token: 'testToken', body: {} })
  expect(actualResponse.body).toEqual(expectedResponse)

  fetchMock.mockResponseOnce('NOT JSON')
  actualResponse = await ApiHelper._patchData({ url: 'https://test.com', token: 'testToken', body: {} })
  expect(actualResponse.body).toEqual(undefined)
})

it('Test patch request Error', async () => {
  fetchMock.mockRejectOnce(new Error('fake error message'))
  const actualResponse = await ApiHelper._patchData({ url: 'https://test.com', token: 'testToken', body: {} })
  expect(actualResponse).toEqual(null)
})
