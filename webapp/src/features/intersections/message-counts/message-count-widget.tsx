import { Box, Card, Container, Grid, TextField, Typography, CardHeader, CardContent } from '@mui/material'
...
export const MessageCountWidget = (props: { accessToken: string | undefined; intersectionId: number }) => {
  ...
  useEffect(() => {
    if (accessToken) {
      const bsmCountPromise = MessageMonitorApi.getMessageCount(accessToken, 'bsm', intersectionId, startDate, endDate)
      bsmCountPromise.then((count) => setBsmCount(count)).catch((error) => console.error(error))
      ...
      spatCountPromise.then((count) => setSpatCount(count)).catch((error) => console.error(error))

      const mapCountPromise = MessageMonitorApi.getMessageCount(accessToken, 'map', intersectionId, startDate, endDate)
      mapCountPromise.then((count) => setMapCount(count)).catch((error) => console.error(error))
    }
