...
export const refreshSnmpFwdConfig = createAsyncThunk(
    ...
    async (ipList, { getState, dispatch }) => {
        ...
        const response = await CdotApi.postRsuData(
            token,
            organization,
            body,
            ''
...
export const filterSnmp = createAsyncThunk(
    'config/filterSnmp',
    async (ipList, { getState, dispatch }) => {
        const currentState = getState()
        const token = selectToken(currentState)
...
export const rebootRsu = createAsyncThunk(
    'config/rebootRsu',
    async (ipList, { getState, dispatch }) => {
        const currentState = getState()
        const token = selectToken(currentState)
...
export const selectRebootChangeSuccess = (state) =>
    state.config.value.rebootChangeSuccess
...
export const { setMsgFwdConfig, setDestIp, setMsgType, togglePointSelect } =
    configSlice.actions
