        ...
        await Promise.all([
            ...
            dispatch(
                _getRsuOnlineStatus({
                    token,
                    organization,
                    rsuOnlineStatusState:
                        currentState.rsu.value.rsuOnlineStatus,
                })
            ),
...
export const getRsuInfoOnly = createAsyncThunk(
    'rsu/getRsuInfoOnly',
    async (_, { getState }) => {
        const currentState = getState()
        const token = selectToken(currentState)
...
export const getRsuLastOnline = createAsyncThunk(
    'rsu/getRsuLastOnline',
    async (rsu_ip, { getState }) => {
        const currentState = getState()
        const token = selectToken(currentState)
        const organization = selectOrganizationName(currentState)
        const rsuLastOnline = await CdotApi.getRsuOnline(
            token,
            organization,
            '',
            { rsu_ip: rsu_ip }
...
const _getRsuInfo = createAsyncThunk(
    'rsu/_getRsuInfo',
...
const _getRsuOnlineStatus = createAsyncThunk(
    ...
    async ({ token, organization, rsuOnlineStatusState }) => {
        const rsuOnlineStatus =
            (await CdotApi.getRsuOnline(token, organization)) ??
            rsuOnlineStatusState
...
export const _getRsuCounts = createAsyncThunk(
    'rsu/_getRsuCounts',
    async (_, { getState }) => {
        const currentState = getState()
        const token = selectToken(currentState)
        ...
        const rsuCounts =
            (await CdotApi.getRsuCounts(
                token,
                organization,
                '',
                query_params
            )) ?? currentState.rsu.value.rsuCounts
...
const _getRsuMapInfo = createAsyncThunk(
    'rsu/_getRsuMapInfo',
    async ({ token, organization, startDate, endDate }) => {
        let local_date = DateTime.local({ zone: 'America/Denver' })
        let localEndDate = endDate === '' ? local_date.toString() : endDate
        let localStartDate =
            startDate === ''
                ? local_date.minus({ days: 1 }).toString()
                : startDate

        const rsuMapData = await CdotApi.getRsuMapInfo(
            token,
            organization,
            '',
            {
...
export const updateRowData = createAsyncThunk(
    ...
    async (data, { getState }) => {
        ...
        const msgType = data.hasOwnProperty('message')
            ? data['message']
            : currentState.rsu.value.msgType
        const startDate = data.hasOwnProperty('start')
            ? data['start']
            : currentState.rsu.value.startDate
        const endDate = data.hasOwnProperty('end')
            ? data['end']
            : currentState.rsu.value.endDate
    ...
    reducers: {
        ...
        updateBsmDate: (state, action) => {
            if (action.payload.type === 'start')
                state.value.bsmStart = action.payload.date
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getRsuLastOnline.fulfilled, (state, action) => {
                state.loading = false
                if (
                    state.value.rsuOnlineStatus.hasOwnProperty(
                        action.payload.ip
                    )
                ) {
                    state.value.rsuOnlineStatus[action.payload.ip][
                        'last_online'
                    ] = action.payload.last_online
                }
            })
            ...
            .addCase(getIssScmsStatus.fulfilled, (state, action) => {
                state.value.issScmsStatusData =
                    action.payload ?? state.value.issScmsStatusData
            })
...
export const selectRsuManufacturer = (state) =>
    state.rsu.value.selectedRsu?.properties?.manufacturer_name
export const selectRsuIpv4 = (state) =>
    state.rsu.value.selectedRsu?.properties?.ipv4_address
export const selectRsuPrimaryRoute = (state) =>
    state.rsu.value.selectedRsu?.properties?.primary_route
...
export const selectIssScmsStatusData = (state) =>
    state.rsu.value.issScmsStatusData
