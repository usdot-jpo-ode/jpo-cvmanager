...
const authLoginData = UserManager.checkLoginData(authDataLocalStorage)
    ...
    reducers: {
        ...
        changeOrganization: (state, action) => {
            state.value.organization =
                UserManager.getOrganization(
                    state.value.authLoginData,
                    action.payload
                ) ?? state.value.organization
        },
...
export const { logout, changeOrganization, setLoading, setLoginFailure } =
    userSlice.actions
